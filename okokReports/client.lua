...
RegisterCommand(Config.FeedbackClientCommand, function(source, args, rawCommand)
	if canFeedback then
		...
		exports['okokNotify']:Alert("REPORT", "No puedes informar tan rápido!", 10000, 'warning')
	end
end, false)
...
exports['okokNotify']:Alert("REPORT", "Nuevo informe Alguien necesita ayuda!", 20000, 'success')
		SendNUIMessage({
			action = "updateFeedback",
			FeedbackTable = FeedbackTable
		})
...
RegisterNUICallback("action", function(data)
	...
	if data.action == "newFeedback" then
		exports['okokNotify']:Alert("REPORT", "Informe enviado con éxito a la PERSONAL!", 20000, 'success')
		...
		if FeedbackTable[data.feedbackid] then
			if oneSync then
				...
				if playerOnline then
					...
					exports['okokNotify']:Alert("REPORT", "Esa jugadora ya no está en el servidor!", 20000, 'error')
				end
			end
		end
