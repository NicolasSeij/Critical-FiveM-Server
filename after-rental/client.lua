...
Citizen.CreateThread(function()
    AddTextEntry('carRental', 'Alquiler De Vehiculos')
    AddTextEntry('carRentalClose', '~INPUT_PICKUP~ Alquiler De Vehiculos')
    while true do
        Wait(0)
        local letSleep = true
...
WarMenu.CreateMenu('carRental', 'Alquiler De Vehiculos')
...
function OpenCarRental(index)
    ...
    while true do
        ...
        if WarMenu.Begin('carRental') then
            ...
            local _, vehicleId = WarMenu.ComboBox('Seleccionar Vehiculo', vehiclesToRent, vehicleIndex)
            ...
            local _, totalTime = WarMenu.ComboBox('Tiempo Del Alquiler', Config.RentalTimes, rentalTimeIndex)
            if rentalTimeIndex ~= totalTime then
                rentalTimeIndex = totalTime
            end

            if WarMenu.CheckBox('Seguro - '..Config.InsurancePrice..'$', insurance) then
                insurance = not insurance
            end

            if WarMenu.IsItemHovered() then
                WarMenu.ToolTip('En caso de que dañes el auto, no pagarás cargos adicionales')
            end
            ...
            if WarMenu.Button('Alquilar Vehiculo', price..'$') then
                if DoesEntityExist(lastLocalVehicle) then
                    DeleteEntity(lastLocalVehicle)
                    lastLocalVehicle = nil
                    FreezeEntityPosition(PlayerPedId(), false)
...
Citizen.CreateThread(function()
    ...
    for k, v in pairs(Config.Points) do
		...
		AddTextComponentSubstringPlayerName('Alquiler De Vehiculos')
...
function createReturnBlips()
    ...
    for k, v in pairs(Config.ReturnPoints) do
		...
		BeginTextCommandSetBlipName('STRING')
		AddTextComponentSubstringPlayerName('Punto de devolución de vehículos')
		EndTextCommandSetBlipName(blip)
...
function startTimer()
    Citizen.CreateThread(function()
        ...
        while rentalTimer>0 do
            ...
            AddTextComponentSubstringPlayerName(disp_time(rentalTimer).." - Tiempo Del Alquiler")
...
Citizen.CreateThread(function()
    AddTextEntry('carReturn', 'Punto de devolución de vehículos')
    AddTextEntry('carReturnClose', '~INPUT_PICKUP~ Devolver Vehiculo')
    while true do
        Wait(0)
        if rentalTimer > 0 then
            local letSleep = true
...
AddEventHandler('sqz_carrental:VehicleSuccessfulyReturned', function()
	...
	PushScaleformMovieMethodParameterString('Alquiler De Vehiculos')
	PushScaleformMovieMethodParameterString('El vehículo ha sido devuelto con éxito')
